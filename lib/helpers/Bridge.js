...
class Bridge {
  constructor(opts) {
    ...
    this.broadcastedTx = new Map();
    this.completedTx = new Map();
    this.failedTx = new Map();
  }
  ...
  appendPendingTransaction = async (network, tx) => {
    if (!this.txQueue.has(network)) {
      this.txQueue.set(network, [tx]);
    } else {
      this.txQueue.get(network).push(tx);
      // If number of appended transactions is greater than the limit defined,
      // execute the pipeline of transactions
      if (this.txQueue.get(network).length > this.txLimit) {
