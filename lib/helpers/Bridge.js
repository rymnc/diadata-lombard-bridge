...
class Bridge {
  constructor(opts) {
    ...
    this.txQueue = {};
    this.completedTx = { source: [], target: [] };
    this.errorTx = { source: [], target: [] };
  }
  ...
  setupEventListeners = async () => {
    if (this.sourceEvents.length >= 1 && this.sourceEvents[0] !== null) {
      this.sourceEvents.forEach((eventName) => {
        this.sourceContract.on(eventName, this.sourceEventHandler);
      });
    }
    if (this.targetEvents.length >= 1 && this.targetEvents[0] !== null) {
  ...
  executePending = async () => {
    ...
    if (source !== 0 && source.length > 0) {
      await this.executeTx(this.sourceContract, "source", source);
    }
    if (target !== 0 && target.length > 0) {
      await this.executeTx(this.targetContract, "target", target);
    }
  };

  /**
   * Executes array of transactions, either on source or target
   * @param {Contract} Contract to send tx from
   * @param {String} Name of network
   * @param {Array} transactions
   */
  executeTx = async (contract, network, transactions) => {
    ...
    transactions.forEach(async (tx) => {
      const payload = this.getPayload(tx);
      try {
        const receipt = await contract.release(...payload);
        const confirmation = await receipt.wait();
        this.completedTx[network].push(confirmation);
        console.log("Confirmed Tx: ", confirmation);
      } catch (e) {
        this.errorTx[network].push(e);
      }
    });
  };

  /**
   * Returns current payload required by the "release" function, as of 23/01/2021
   * @param {Event} Event Object from ethers.js lib
   */
  getPayload = (tx) => {
    return [tx.targetAddress, tx.tokenContractAddress, tx.amount];
  };
}
