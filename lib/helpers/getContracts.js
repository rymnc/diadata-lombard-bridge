...
const getContracts = (provider) => {
  ...
  contracts.forEach((contract_pair) => {
    const sourceAbi = JSON.parse(
      fs.readFileSync(contract_pair.pathToSourceArtifact)
    );
    const destAbi = JSON.parse(
      fs.readFileSync(contract_pair.pathToDesArtifact)
    );
    bridgeContracts.set(contract_pair.name, {
      ...
      destinationContract: {
        ...
        contract: new ethers.Contract(
          contract_pair.destinationContract,
          destAbi,
          provider
        ),
      },
